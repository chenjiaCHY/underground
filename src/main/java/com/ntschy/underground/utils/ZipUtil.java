...
public class ZipUtil {
     ...
     */
    public static void unzip(String zipFilePath, String unzipDir, boolean includeZipFileName) {
        if (isEmpty(zipFilePath) || isEmpty(unzipDir)) {
            throw new RuntimeException("空参数");
        }
        zipFilePath = zipFilePath.replace("/", File.separator);
        unzipDir = unzipDir.replace("/", File.separator);
        File zipFile = new File(zipFilePath);
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                entryFilePath = unzipDir + File.separator + entry.getName();
                // 构建解压后保存的文件夹路径
